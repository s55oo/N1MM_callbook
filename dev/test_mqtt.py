...
def main():
    ...
    tls_calls = [c for c in fake.calls if c[0] == "tls"]
    ok &= check(
        "relative CA path resolved next to the config file",
        len(tls_calls) == 1
        and os.path.isabs(tls_calls[0][1])
        and os.path.basename(tls_calls[0][1]) == "broker-ca.pem"
        and os.path.basename(os.path.dirname(tls_calls[0][1])) == "config",
    )
    ...
    cached_app.lookup_chain = (mc, mc)  # length 2, matching rows[:2]
    ...
    ok &= check(
        "live cache key is the bare call (LAN peers share it across views)",
