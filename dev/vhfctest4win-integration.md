...
**Status: shipped.** The 6767 feed lives in **`Callbooker`** (on by
default, `vhfctest_share=no` to disable). It has been through several app
shapes on the way here (`VHFctest4WinCallbook`, then a merged
`VHFcallbook`, now folded into the single `Callbooker`); this file is the
protocol / port background. User-facing docs: README section 1 and
`CLAUDE.md`.
...
3. `Callbooker.main()` bridges 1→2: if the 6767 feed is wanted, 6767 is
...
- `CallbookApp`: `_v4w_inbox` / `_on_v4w_call(call, src)` / `_on_v4w_status`
  ...
  `127.0.0.1`.
- `VHFCTEST_CAPABLE` (True on `CallbookerApp`) + `run(always_vhfctest=...)`.
- `Callbooker.py` — `CallbookerApp(cb.CallbookApp)` + the UAC
  self-relaunch; `main()` computes `always_vhfctest` from `vhfctest_share`
  (default yes).
